...
var SubCategory = /** @class */ (function () {
  ...
  SubCategory.prototype.addNode = function (current) {
    ...
    if (this.data.has(type)) {
      ...
      console.log(
        "Sub-category ".concat(subCategory, " failed to add ").concat(type, ".")
      );
    }
  };
  /* Helper Functions */
  ...
  // Returns number of types that exist within this sub-category
  SubCategory.prototype.getNumChildren = function () {
    return this.numTypes;
  };
  // Returns number of .csv rows that were grouped into this specific sub-category
  SubCategory.prototype.getNumRows = function () {
    var _this = this;
    var res = 0;
    this.data.forEach(function (value, key) {
      return (res += _this.data.get(key).getDataSize());
    });
    return res;
  };
  return SubCategory;
