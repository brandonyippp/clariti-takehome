...
var Manager = /** @class */ (function () {
  ...
  Manager.prototype.establishTotals = function (levelObj) {
    ...
    console.log(levelObj.constructor);
    console.log(levelObj.getTotal() + "\n");
    return res;
  };
  ...
  Manager.prototype.addDepartment = function (current) {
    this.data.set(
      ...
      new Department_1.Department(current, this.invalidData)
    );
    this.numDepartments++;
  };
  Manager.prototype.processInvalidData = function (current) {
    this.invalidData.push(
      __assign(__assign({}, current), {
        reason: "Department "
          .concat(current.Department__c, " not any of ")
          .concat(
            (0, constants_1.getAllEnumValuesAsString)(constants_1.Departments),
            "."
          ),
      })
    );
  ...
  Manager.prototype.printInvalidDataFields = function (i) {
    console.log(
      "Id: "
        .concat(this.invalidData[i].Id, ", Name: ")
        .concat(this.invalidData[i].Name, ", Reason: ")
        .concat(this.invalidData[i].reason)
    );
  };
  /* Getters & Setters */
